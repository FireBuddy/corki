...
// Use Items
		...
		if ((item && Botrk2.IsReady() && Botrk2.IsOwned() && target.IsValidTarget(700)) && (Player.Instance.HealthPercent <= Minhp || target.HealthPercent < Minhpp))
                {
                    Botrk2.Cast(target);
                }
            }
